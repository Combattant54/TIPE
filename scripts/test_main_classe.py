...
def main():
    sim = Simulation(objectifs=OBJECTIFS, rect_obstacles=OBSTACLE_RECTS, taille=[10, 10], resolution=10, radius = RADIUS)
    sim.init_foule(build_foule([RADIUS] * len(POS), POS))
    
    steps = 0
    t0 = time.time()
    while (not sim.is_finished()) and (steps < 1000/dt):
        steps += 1
